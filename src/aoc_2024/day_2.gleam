...
pub type Reports = List(List(Int))
...
fn sorted_report_check(report: List(Int)) {
  ...
  let reversed_sorted_report = list.sort(report, int.compare)
    |> list.reverse()
...
fn difference_check(differences: List(Int), report: List(Int)) {
  let difference_lower_than_one_larger_than_three = differences
...
fn process_report_without_specific_level(report: List(Int)) {
  ...
  let safe_or_not = level_range
    |> list.any(fn (level) {
      let report_without_level_as_index = create_report_without_level(report, level)
