...
fn get_all_trailheads_positions(topographic_map: TopographicMap) {
  let trailheads = topographic_map
  |> list.index_fold([], fn (final_accumulator, row, row_index) {
    let row_col_pair = row
...
fn find_end_of_trail(row: Int, col: Int, topographic_map: TopographicMap, visited: set.Set(#(Int, Int)), previous_value: Int) {
      ...
      case previous_value + 1 != current_value {
        ...
        False -> {
          case current_value == 9 {
            ...
            False -> {
              let #(visited_above_row, value_above_row) = find_end_of_trail(row - 1, col, topographic_map, visited, current_value)
              let #(visited_below_row, value_below_row) = find_end_of_trail(row + 1, col, topographic_map, visited, current_value)
              let #(visited_left_col, value_left_col) = find_end_of_trail(row, col - 1, topographic_map, visited, current_value)
              let #(visited_right_col, value_right_col) = find_end_of_trail(row, col + 1, topographic_map, visited, current_value)

              #(visited |> set.union(visited_above_row) |> set.union(visited_below_row) |> set.union(visited_left_col) |> set.union(visited_right_col), value_above_row + value_below_row + value_left_col + value_right_col)
            }
          }
        }
      }
...
pub fn pt_1(topographic_map: TopographicMap) {
  ...
  let sum_of_trailheads = trailheads
  |> list.fold(0, fn (accumulator, trailhead) {
    let #(row, col) = trailhead

    let #(visited, _) = find_end_of_trail(row, col, topographic_map, set.new(), -1)
...
pub fn pt_2(topographic_map: TopographicMap) {
  ...
  let sum_of_trailheads_ratings = trailheads
  |> list.fold(0, fn (accumulator, trailhead) {
    let #(row, col) = trailhead

    let #(_, rating) = find_end_of_trail(row, col, topographic_map, set.new(), -1)
